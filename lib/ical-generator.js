			...
			methods: {
				...
				setTZ: function(tz) {
					if (typeof tz === 'string') {
						...
						cal.tz.offset = tz.offset || null;
						cal.tz.region = tz.region || null;
					}
				...
				generate: function() {
					...
					function _formatDateProperty(side, e, timezone) {
						var label = (side === 'start') ? 'DTSTART' : 'DTEND';
						...
						return label + ':' + _formatDate(e[side], e) + '\n';
					}
					...
					cal.events.forEach(function(e) {
						...
						g+= _formatDateProperty('start', e, cal.tz.region);
						g+= _formatDateProperty('end', e, cal.tz.region);
